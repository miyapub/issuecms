...
if(strpos($_SERVER['PHP_SELF'], "comments")>0){
    $comments_on="on";
}
...
if(strpos($_SERVER['PHP_SELF'], "notify")>0){
    $notify_on="on";
}
...
$result = mysql_query($sql,$conn);
$rows = mysql_fetch_array($result);
$TotalComments=$rows['count'];

$sql="select count(id) as count from msgs";
$result = mysql_query($sql,$conn);
$rows = mysql_fetch_array($result);
$TotalMsgs=$rows['count'];

?>
      <ul class="menu">
        <li class="menu-item <?=$index_on?>">
          <a href="index.php">消息</a>
        </li>
        <li class="menu-item <?=$comments_on?>">
...
<a href="news.php">新鲜事儿</a>
        ...
        <li class="menu-item <?=$device_on?>">
          <a href="projects.php?tag=device">设备</a>
        </li>

        <li class="menu-item <?=$company_on?>">
          <a href="projects.php?tag=company">公司</a>
        </li>

        <li class="menu-item <?=$event_on?>">
          <a href="projects.php?tag=event">事件</a>
        </li>
        ...
        <li class="menu-item <?=$users_on?>">
          <a href="users.php">财富排行榜</a>
        </li>
        <li class="menu-item <?=$notify_on?>">
          <a href="notify.php">免责声明</a>
        </li>
