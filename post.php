...
if($method==='POST'){
    ...
    $sql="INSERT INTO `articles` (`id`,`issue_id`, `title`, `quote`,`headpic`, `text`, `time`, `user_id`,`display`, `viewed`) VALUES (NULL, '$issue','$title','$quote', '$file', '$text', CURRENT_TIMESTAMP, '$user_id','$display', '0');";
            ...
            <form method="post" target="msg" enctype="multipart/form-data">
              ...
              <div>
                ...
                if(isset($_SESSION['user_name'])){
                  ?>
                  作者：<?=$_SESSION['user_name']?>
                  <?
                }else{
                  ?>作者：匿名用户（如果想署名发布文章，请先登录）<?
                }
                ?>
              </div>
              <div>
              ...
              <div>
                <textarea placeholder="引用" name="quote"></textarea>
                <textarea placeholder="正文" name="text"></textarea>
                <script>
                tinymce.init({
